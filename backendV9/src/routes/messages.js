...
router.get('/conversations/:id', requireAuth, async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT m.*, m.text as content, u.name AS sender_name, u.avatar_url AS sender_avatar
       FROM messages m
...
router.post('/conversations/:id', requireAuth, async (req, res, next) => {
  try {
    ...
    if (!finalContent) return res.status(400).json({ message: 'content or attachment required' });
  ...
  try {
    const { rows: [{ count }] } = await pool.query(
      `SELECT COUNT(*) FROM messages m
       ...
       WHERE (c.client_id=$1 OR c.lawyer_id=$1) AND m.sender_id!=$1 AND m.is_read=FALSE`,
      [req.user.id]
    );
