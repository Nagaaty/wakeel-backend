...
router.post('/:id/review', requireAuth, async (req, res, next) => {
  try {
    ...
    const { rows: updatedRows } = await pool.query(
      `UPDATE reviews SET rating=$1, comment=$2, outcome=$3 
       WHERE client_id=$4 AND lawyer_id=$5 RETURNING *`,
      [rating, comment||null, outcome||null, req.user.id, req.params.id]
    );
...
router.get('/me/reviews', requireAuth, async (req, res, next) => {
  try {
    const { rows } = await pool.query(
      `SELECT r.*, u.name as client_name, b.type as service_type
       FROM reviews r
