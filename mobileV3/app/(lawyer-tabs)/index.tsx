    ...
    <View>
      ...
      <View style={{ flexDirection: isRTL ? 'row-reverse' : 'row', flexWrap: 'wrap', gap: 10, marginBottom: 24 }}>
        {[
          ...
          { icon: '📍', label: isRTL ? 'موقع المكتب' : 'Office Location', href: '/edit-profile' },
        ].map((item, i) => (
          <TouchableOpacity
            key={i}
        ...
        <TouchableOpacity
          onPress={() => router.push('/my-postings')}
          style={{
            width: '100%', backgroundColor: '#0F172A',
            borderWidth: 1, borderColor: C.gold + '50', borderRadius: 16,
            padding: 20, flexDirection: 'row', alignItems: 'center', gap: 14, marginTop: 4,
            shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 4
          }}>
          <View style={{ backgroundColor: '#1E293B', padding: 12, borderRadius: 12 }}>
            <Text style={{ fontSize: 24 }}>🎯</Text>
          </View>
          <View style={{ flex: 1 }}>
            <Text style={{ color: '#F8FAFC', fontWeight: '800', fontSize: 16 }}>{isRTL ? 'التوظيف الاحترافي' : 'Professional Recruiting'}</Text>
            <Text style={{ color: '#94A3B8', fontSize: 12, marginTop: 4 }}>{isRTL ? 'نشر الوظائف وجذب أفضل الكفاءات' : 'Post jobs and attract top talent'}</Text>
          </View>
          <View style={{ backgroundColor: C.gold, paddingHorizontal: 14, paddingVertical: 6, borderRadius: 20 }}>
            <Text style={{ color: '#fff', fontSize: 12, fontWeight: '800' }}>{isRTL ? 'ابدأ' : 'Start'}</Text>
          </View>
        </TouchableOpacity>
      </View>
