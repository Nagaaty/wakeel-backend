...
export default function ProfileTab() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      >
        {/* ── HERO COVER + AVATAR — direction:ltr ensures physical-left positioning on RTL ── */}
        <View style={{ direction: 'ltr' } as any}>
          ...
          {/* Avatar — always physical LEFT, Facebook style */}
          <TouchableOpacity
            activeOpacity={0.85}
            onPress={() => {
            ...
            style={{
              ...
              bottom: -44,
              left: 16,
              // Force physical-left on RTL Android devices:
              direction: 'ltr',
            } as any}>
        ...
        <View style={{ paddingTop: 56, paddingHorizontal: 20, paddingBottom: 18, backgroundColor: C.surface, borderBottomWidth: 1, borderBottomColor: C.border }}>
          ...
          {/* ── Privacy indicator — taps to Account Settings ── */}
          {!isLawyer && (
            <TouchableOpacity
              ...
              style={{
                ...
                borderRadius: 20,
                backgroundColor: user?.forum_activity_public ? '#22C55E18' : C.border,
                alignSelf: 'flex-start',
              }}
            >
              <Text style={{ fontSize: 13 }}>
                {user?.forum_activity_public ? '🔓' : '🔒'}
              </Text>
              <Text style={{ color: user?.forum_activity_public ? '#22C55E' : C.muted, fontSize: 12, fontWeight: '700' }}>
                {user?.forum_activity_public
                  ? (isRTL ? 'ملفك الشخصي عام' : 'Public profile')
                  : (isRTL ? 'ملفك الشخصي خاص' : 'Private profile')}
              </Text>
              <Text style={{ color: C.muted, fontSize: 11 }}>· {isRTL ? 'تغيير' : 'Change'}</Text>
            </TouchableOpacity>
          )}
        </View>
        ...
        {/* ── MY LAWYERS — horizontal scroll, LinkedIn "Connections" style ── */}
        <View style={{ backgroundColor: C.surface, borderTopWidth: 1, borderBottomWidth: 1, borderColor: C.border, marginTop: 8, paddingTop: 16, paddingBottom: 4 }}>
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, marginBottom: 14 }}>
            <Text style={{ color: C.text, fontSize: 20, fontWeight: '800', fontFamily: 'Cairo-Bold' }}>
              {isRTL ? '⚖️ محاميّ' : '⚖️ My Lawyers'}
            </Text>
            <TouchableOpacity onPress={() => router.push('/(tabs)/lawyers' as any)}
              style={{ borderWidth: 1, borderColor: C.gold, borderRadius: 20, paddingHorizontal: 14, paddingVertical: 5 }}>
              <Text style={{ color: C.gold, fontSize: 13, fontWeight: '700' }}>
                {isRTL ? 'تصفح المحامين' : 'Find lawyers'}
              </Text>
            </TouchableOpacity>
          </View>
          ...
          {favLawyers.length === 0 ? (
            /* Empty state */
            <View style={{ alignItems: 'center', paddingVertical: 28, paddingHorizontal: 24, gap: 10 }}>
              <Text style={{ fontSize: 36 }}>👨‍⚖️</Text>
              <Text style={{ color: C.muted, fontSize: 14, textAlign: 'center', lineHeight: 20 }}>
                {isRTL
                  ? 'لم تضف محامياً مفضلاً بعد.\nاضغط لتصفح أفضل المحامين.'
                  : "You haven't added any favourite lawyers yet.\nBrowse and find your match."}
              </Text>
              <TouchableOpacity onPress={() => router.push('/(tabs)/lawyers' as any)}
                style={{ backgroundColor: C.gold, borderRadius: 10, paddingHorizontal: 22, paddingVertical: 9, marginTop: 4 }}>
                <Text style={{ color: '#fff', fontWeight: '800', fontSize: 13 }}>
                  {isRTL ? '🔍 ابحث عن محامٍ' : '🔍 Find a Lawyer'}
                </Text>
              </TouchableOpacity>
            </View>
          ) : (
            ...
            >
              {favLawyers.map((lawyer: any) => {
                const lawyerName = lawyer.lawyer_name || lawyer.name || '—';
                const spec = lawyer.specialization || lawyer.lawyer_specialization || '';
                const rating = lawyer.average_rating ?? lawyer.rating ?? null;
                const avatarUri = lawyer.lawyer_avatar_url || lawyer.avatar_url;
                const lawyerInitials = lawyerName.trim().split(/\s+/).map((w: string) => w[0]).join('').slice(0, 2).toUpperCase();
                const lawyerId = lawyer.lawyer_id || lawyer.id;
                  ...
                  >
                    ...
                    {avatarUri ? (
                      <Image
                        ...
                        style={{ width: 64, height: 64, borderRadius: 32, borderWidth: 2.5, borderColor: C.gold + '60' }}
                      />
                    ) : (
                      <View style={{
                        width: 64, height: 64, borderRadius: 32,
                        backgroundColor: C.gold + '22',
                        borderWidth: 2.5, borderColor: C.gold + '60',
                        alignItems: 'center', justifyContent: 'center',
                      }}>
                        <Text style={{ color: C.gold, fontSize: 20, fontWeight: '800' }}>{lawyerInitials}</Text>
                      </View>
                    )}

                    {/* Name */}
                    <Text style={{ color: C.text, fontSize: 13, fontWeight: '700', textAlign: 'center', lineHeight: 17 }} numberOfLines={2}>
                      {lawyerName}
                    </Text>

                    {/* Specialization */}
                    {!!spec && (
                      <Text style={{ color: C.gold, fontSize: 11, textAlign: 'center', fontWeight: '600' }} numberOfLines={1}>
                        {spec}
                      </Text>
                    )}
              ...
              >
                ...
                <Text style={{ color: C.muted, fontSize: 11, textAlign: 'center', fontWeight: '600' }}>
                  {isRTL ? 'تصفح\nالمزيد' : 'Browse\nmore'}
                </Text>
              </TouchableOpacity>
            </ScrollView>
          ...
          {myPostsLoading ? (
            ...
            const posts = myPosts
              .map((p: any) => {
                ...
                // Strip out chained repost tags like "[إعادة نشر من lawyer 1]:"
