...
import { PostSkeleton } from '../../src/components/forum/PostSkeleton';
import { ForumSearchBar } from '../../src/components/forum/ForumSearchBar';
import { HashtagSuggestions } from '../../src/components/forum/HashtagSuggestions';
import { NewPostsPill } from '../../src/components/forum/NewPostsPill';
...
export default function ForumTab() {
  ...
  const feedBg = '#EBEDF0';
  // Route params — used by Chunk 3c "Ask this lawyer" deep-link
  ...
  // ── Pending posts buffer (Chunk 3c) ───────────────────────────────────────
  // When a `forum:new_post` socket event arrives, we DON'T inject the post
  // into the feed immediately (that would shift the user's reading position).
  // Instead we stash it in `pendingPosts` and surface a tappable pill at the
  // top of the screen. When the user taps the pill, we drain the buffer into
  // the main `posts` array and scroll to top.
  //
  // We also dedupe by id — if a post somehow lands in pendingPosts AND in a
  // page-refresh batch, we keep only one copy.
  const [pendingPosts, setPendingPosts] = useState<any[]>([]);
  const flatListRef = useRef<any>(null);

  // ── Hashtag autocomplete draft (Chunk 3b) ────────────────────────────────
  // Detect the in-progress hashtag — the last `#word` at the END of the
  // current text. Returns the tag chars (no `#`) if active, or null
  // otherwise. The autocomplete dropdown only renders while this is non-null.
  //
  // Why "at the end"? It's the simplest and most reliable heuristic without
  // tracking the TextInput cursor (which RN doesn't expose reliably across
  // platforms). Matches Twitter/Facebook/LinkedIn compose UX.
  const hashtagDraft = useMemo<string | null>(() => {
    if (!newPostText) return null;
    // Match a #word at end of string. Accepts Arabic, Latin, digits, underscore.
    // The hashtag is "active" only if no whitespace follows it (i.e. user is
    // still typing it).
    const m = newPostText.match(/(?:^|\s)#([\w\u0600-\u06FF_]*)$/);
    if (!m) return null;
    return m[1] || ''; // empty string when user just typed `#` with nothing after
  }, [newPostText]);

  const handleHashtagPick = useCallback((tag: string) => {
    // Replace the trailing `#partial` with `#picked ` (trailing space).
    setNewPostText(prev => prev.replace(/#([\w\u0600-\u06FF_]*)$/, `#${tag} `));
  }, []);
  ...
  const [hideMediaUI, setHideMediaUI]             = useState(false);
  // Post ··· menu state
  ...
  useForumSocket({
    ...
    onNewPost: ({ post }) => {
      // Don't inject directly — buffer in pendingPosts so the "X new posts"
      // pill can surface it. The user taps to inject + scroll. Skip if it's
      // the user's own post (they already see it via optimistic UI on submit).
      if (post.user_id && user?.id && post.user_id.toString() === user.id.toString()) {
        return;
      }
      setPendingPosts(prev => {
        // Dedupe against both pending buffer AND the rendered list
  ...
  // ── Drain pending posts ────────────────────────────────────────────────
  // Called when user taps the "X new posts" pill: prepends pending posts
  // to the main list, clears the buffer, scrolls to top.
  const drainPending = useCallback(() => {
    ...
    setPendingPosts([]);
    // Scroll to the very top so the user sees the new posts.
    setTimeout(() => {
  ...
  // Mount: single merged feed request (3-in-1)
  useEffect(() => {
    loadFeed(sortMode);
  }, []);

  // ── "Ask this lawyer" deep-link handler (Chunk 3c) ──────────────────────
  // Triggered when the user lands on /(tabs)/forum?askLawyer=Lawyer+Name
  // (typically from the AskLawyerButton on a lawyer profile). Opens the
  // compose modal with `@LawyerName ` already in the input so the user can
  // continue typing their question.
  //
  // We read the param via useLocalSearchParams (already imported). After
  // consuming, we use router.setParams to clear the param so navigating
  // back-forward doesn't re-trigger.
  const askLawyerParam = (params as any)?.askLawyer as string | undefined;
  useEffect(() => {
    if (!askLawyerParam) return;
    const name = String(askLawyerParam).trim();
    if (!name) return;
    setNewPostText(prev => {
      // Don't double-prefill if user has already started a draft with the mention
      if (prev.startsWith(`@${name} `) || prev.startsWith(`@${name}\n`)) return prev;
      return `@${name} ${prev}`;
    });
    setModalOpen(true);
    // Clear the param so a back-navigation won't re-open the modal
    try {
      router.setParams({ askLawyer: undefined } as any);
    } catch {
      // setParams not available in some expo-router versions — non-fatal
    }
  }, [askLawyerParam]);

  const loadPosts = useCallback(async (sort: string = sortMode) => {
    try {
      setLoading(true);
      setNextCursor(null);
      setHasMore(true);
      if (user?.id) {
        const [res, savedRes]: any = await Promise.all([
          forumAPI.getQuestions({ sort, limit: 20 }),
        ...
        const res: any = await forumAPI.getQuestions({ sort, limit: 20 });
        setPosts(res.questions || []);
        setNextCursor(res.next_cursor || null);
        setHasMore(!!res.has_more);
      }
  ...
  // Infinite scroll — fetch the next page when user nears the bottom.
  // Cursor pagination is only available for the 'new' sort (backend constraint
  // — score-based sorts are unstable mid-pagination). For other sorts this
  // becomes a no-op, which is fine because the initial load already returns
  // 50 results.
  const loadMore = useCallback(async () => {
    if (loadingMore || loading || !hasMore || !nextCursor) return;
    if (sortMode !== 'new') return; // only 'new' supports cursor today
    try {
      setLoadingMore(true);
      ...
      const newPosts = res.questions || [];
      // Defensive: filter out any IDs that are already in the list
      // (shouldn't happen with correct cursors, but guards against duplicates)
      setPosts(prev => {
        const seen = new Set(prev.map((p: any) => p.id));
        return [...prev, ...newPosts.filter((p: any) => !seen.has(p.id))];
      });
      setNextCursor(res.next_cursor || null);
      setHasMore(!!res.has_more);
    } catch (e) {
      console.warn('[forum] loadMore failed', e);
    } finally {
      setLoadingMore(false);
    }
  }, [loadingMore, loading, hasMore, nextCursor, sortMode]);

  // ── Stable handlers for FlatList renderItem ───────────────────────────────
  // Keeping renderItem at the function-body level (not inside JSX) so its
  // identity is stable across renders. This is critical: a new function
  // identity invalidates every PostCard's React.memo, which destroys
  // virtualization performance.
  const onViewableItemsChanged = useCallback(({ viewableItems }: any) => {
    DeviceEventEmitter.emit('visible_posts', viewableItems.map((v: any) => v.item.id));
  }, []);

  // Fire an initial visibility event after posts load so the first card's
  // video plays immediately without requiring a scroll gesture.
  useEffect(() => {
    if (posts.length > 0) {
      const timer = setTimeout(() => {
        const firstIds = posts.slice(0, 3).map((p: any) => p.id);
        DeviceEventEmitter.emit('visible_posts', firstIds);
      }, 600);
      return () => clearTimeout(timer);
    }
  }, [posts.length > 0]);

  // Tighter threshold: 50% visible = item is meaningfully on screen
  ...
  // Footer = loading-more spinner OR end-of-list message
      ...
      return (
        <View style={{ paddingVertical: 24, alignItems: 'center' }}>
          <Text style={{ color: C.muted, fontSize: 12 }}>
            {isRTL ? '— لا مزيد من المنشورات —' : '— No more posts —'}
          </Text>
        </View>
      );
    }
    return null;
  }, [loadingMore, hasMore, posts.length, C.muted, C.gold, isRTL]);

  // Stable comma-separated lists of liked/disliked/saved IDs.
  // Using JSON strings means renderItem only gets a new identity when the
  // actual set of liked post IDs changes — not on every render.
  const likedKey   = useMemo(() => [...likedPosts].sort().join(','),   [likedPosts]);
  const dislikedKey= useMemo(() => [...dislikedPosts].sort().join(','),[dislikedPosts]);
  const savedKey   = useMemo(() => [...savedPosts].sort().join(','),   [savedPosts]);

  const renderItem = useCallback(({ item: p }: any) => {
    if (p.isHidden) {
      return (
        <View style={{ marginHorizontal: 16, marginBottom: 8, padding: 16, backgroundColor: '#FFF', borderRadius: 12, flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 5, elevation: 2 }}>
          <View style={{ flexDirection: 'row-reverse', alignItems: 'center', gap: 10 }}>
            ...
            <Text style={{ color: '#65676B', fontSize: 14, fontWeight: '600' }}>{isRTL ? 'تم إخفاء هذا المنشور.' : 'Post hidden.'}</Text>
          </View>
          <TouchableOpacity onPress={() => setPosts(prev => prev.map(x => x.id === p.id ? { ...x, isHidden: false } : x))}>
            <Text style={{ color: '#0A66C2', fontSize: 14, fontWeight: '700' }}>{isRTL ? 'تراجع' : 'Undo'}</Text>
          </TouchableOpacity>
        </View>
      );
    }
    ...
    C, user, isRTL,
    likedKey, dislikedKey, savedKey,
    likedPosts, dislikedPosts, savedPosts,
    handleLike, handleDislike, handleSave,
    openComments, handleShare, handleNativeShare,
    openReactors,
  ]);
  ...
  // ⚡ MERGED FEED LOADER — replaces 3 parallel calls with 1 (Facebook/LinkedIn pattern)
  const loadFeed = useCallback(async (sort: string = 'hot') => {
    try {
      setLoading(true);
      setNextCursor(null);
      setHasMore(true);

      const [feedRes, savedRes]: any = await Promise.all([
        forumAPI.getFeed({ sort, limit: 20 }),
        user?.id ? forumAPI.getSavedPosts().catch(() => ({ questions: [] })) : Promise.resolve({ questions: [] }),
      ]);

      setPosts(feedRes.questions || []);
      setNextCursor(feedRes.next_cursor || null);
      setHasMore(!!feedRes.has_more);
      if (feedRes.stats)    setStats(feedRes.stats);
      if (feedRes.trending) setTrending(feedRes.trending);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: feedBg }}>
      ...
      {/* Floating "X new posts" pill — Chunk 3c */}
      <NewPostsPill count={pendingPosts.length} onPress={drainPending} />
      ...
      <Animated.FlatList
        ...
        refreshing={loading}
        onRefresh={() => { setSortMode('hot'); loadFeed('hot'); }}
        ...
        ListHeaderComponent={
          <>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFF', borderBottomWidth: 1, borderColor: '#EBEDF0', marginBottom: 8 }}>
              {/* Filter Tabs */}
              <View style={{ flex: 1, flexDirection: 'row' }}>
                {([
                  ...
                  <TouchableOpacity key={mode}
                    disabled={loading}
                    onPress={() => { setSortMode(mode); loadFeed(mode); }}
                    activeOpacity={0.6}
                    style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
                      gap: 4, paddingVertical: 12,
                      opacity: loading && sortMode !== mode ? 0.5 : 1,
                      borderBottomWidth: 3,
          ...
          loading ? (
            ...
            <View style={{ alignItems: 'center', paddingVertical: 60 }}>
              ...
              <Text style={{ color: C.muted, fontSize: 13 }}>
                {isRTL ? 'كن أول من يشارك سؤالاً!' : 'Be the first to ask a question!'}
              </Text>
            </View>
          )
        }
      ...
      <Modal visible={modalOpen} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setModalOpen(false)}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1, backgroundColor: C.surface }}>
          ...
          {/* Hashtag autocomplete (Chunk 3b) — appears while user is typing
              a #word at the cursor. Tapping a suggestion replaces the partial
              hashtag with the picked one + trailing space. */}
          {hashtagDraft !== null && (
            <HashtagSuggestions
              query={hashtagDraft}
              ...
              {['الكل', 'جنائي', 'تجاري', 'عقاري', 'أسرة', 'عمالي', 'إداري', 'دستوري'].map(cat => {
                ...
                return (
                  <TouchableOpacity key={cat} onPress={() => setPostCategory(cat)}
                    style={{
                      height: 32, paddingHorizontal: 16, borderRadius: 16, flexShrink: 0,
                      backgroundColor: active ? C.gold : (C.isDark ? '#333' : '#F3F4F6'),
                      borderWidth: 1, 
                      borderColor: active ? C.gold : (C.isDark ? '#444' : '#E5E7EB'),
                      alignItems: 'center', justifyContent: 'center',
                    }}>
                    <Text style={{ fontSize: 13, fontWeight: active ? '800' : '600', color: active ? '#fff' : C.text, fontFamily: active ? 'Cairo-Bold' : 'Cairo-SemiBold' }}>{cat}</Text>
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
          ...
          {attachedImage && !uploading && (
            <View style={{ marginHorizontal: 20, marginBottom: 12, position: 'relative' }}>
              {attachedImage.toLowerCase().match(/\.(mp4|mov|webm|m4v)(\?.*)?$/) ? (
                <View style={{ width: '100%', height: 200, borderRadius: 12, overflow: 'hidden', backgroundColor: '#000' }}>
                  <Video
              ...
              <TouchableOpacity onPress={() => { setAttachedImage(null); setAttachedFile(null); }}
                style={{ position: 'absolute', top: 8, right: 8, backgroundColor: 'rgba(0,0,0,0.6)', borderRadius: 14, width: 28, height: 28, alignItems: 'center', justifyContent: 'center', zIndex: 10 }}>
          ...
          {/* Bottom Toolbar */}
          <View style={{ padding: 16, borderTopWidth: 1, borderTopColor: C.border, flexDirection: 'row', alignItems: 'center', gap: 20 }}>
      ...
      <Modal 
        ...
        transparent 
        animationType="fade" 
        onRequestClose={() => { setFullscreenMedia(null); setHideMediaUI(false); }}
      >
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.98)', justifyContent: 'center' }}>
          ...
          <View style={{ position: 'absolute', top: insets.top || 20, left: 0, right: 0, zIndex: 10, flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16 }}>
            <TouchableOpacity onPress={() => { setFullscreenMedia(null); setHideMediaUI(false); }} hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }} style={{ width: 40, height: 40, alignItems: 'center', justifyContent: 'center' }}>
          ...
          {/* Media Content */}
          <TouchableOpacity activeOpacity={1} onPress={() => setHideMediaUI(!hideMediaUI)} style={{ flex: 1 }}>
