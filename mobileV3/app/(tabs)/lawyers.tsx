...
const LawyerCardGrid = memo(function LawyerCardGrid({ lawyer, C, onBook, onProfile, isFav, onToggleFav }: any) {
  ...
  return (
    <View style={{
      ...
      <Text style={{ color:C.muted, fontSize:11, textAlign: 'center', marginBottom: lawyer.office ? 2 : 6 }} numberOfLines={1}>{lawyer.specialization} • {lawyer.city}</Text>
      {lawyer.office && (
        <Text style={{ color:C.muted, fontSize:10, textAlign: 'center', marginBottom: 6 }} numberOfLines={1}>📍 {lawyer.office}</Text>
      )}
