...
type Tab = 'about' | 'posts' | 'activity' | 'reviews' | 'availability';
...
export default function LawyerPublicProfile() {
  ...
  const handleMessage = useCallback(() => {
    ...
    // Conversation creation happens server-side on first message — we just
    // navigate to a deep-link the messages screen handles
    router.push({ pathname: '/messages', params: { lawyerId: id } } as any);
  }, [isLoggedIn, id]);
    ...
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      >
        ...
        <ProfileHeader
          ...
          secondaryActions={[
            // Book button is sticky at the bottom — we don't double-stack it here
          ]}
        />
        ...
        <View style={[styles.tabStrip, { backgroundColor: C.surface, borderBottomColor: C.border }]}>
          ...
          >
            {(['about', 'posts', 'activity', 'reviews', 'availability'] as Tab[]).map(t => (
              <TouchableOpacity
                key={t}
                onPress={() => setTab(t)}
                style={[
              ...
              >
                <Text style={{
                  ...
                  : t === 'posts'       ? (isRTL ? 'منشورات'    : 'Posts')
                  : t === 'activity'    ? (isRTL ? 'النشاط'     : 'Activity')
        ...
        <View style={{ paddingTop: 4, minHeight: 400 }}>
            ...
            <View>
              {pinned.length > 0 && (
                <View style={{ paddingTop: 10 }}>
                  {pinned.map(p => <PinnedPostCard key={p.id} post={p} />)}
                </View>
              )}
          ...
          {/* ACTIVITY */}
          {tab === 'activity' && (
            <ActivityFeed userId={lawyer.id} scrollEnabled={false} />
          )}
