...
router.patch('/me', requireAuth, async (req, res, next) => {
  try {
    const { name, phone, bio, avatar_url, cover_url, forum_activity_public } = req.body;
    // Defensive: ensure the privacy column exists (migration 005)
    await pool.query(
      `ALTER TABLE users ADD COLUMN IF NOT EXISTS forum_activity_public BOOLEAN DEFAULT false`
    ).catch(() => {});
    ...
    const { rows: [user] } = await pool.query(
      `UPDATE users SET
         ...
         forum_activity_public = COALESCE($7, forum_activity_public)
       WHERE id=$6 RETURNING *`,
      [name, phone, bio, avatar_url, cover_url, req.user.id,
       typeof forum_activity_public === 'boolean' ? forum_activity_public : null]
    );
