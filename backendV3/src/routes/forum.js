...
// GET /api/forum/feed — merged feed (questions + stats + trending in one request)
// The mobile forum screen calls this instead of three separate requests.
router.get('/feed', async (req, res, next) => {
  try {
    const { cat, search, sort = 'new', limit } = req.query;
    const pageSize = Math.min(Math.max(parseInt(limit) || 20, 5), 50);
    ...
    // Run questions + stats + trending in parallel
    const [questionsResult, statsResult, trendingResult] = await Promise.all([
      pool.query(q, params),
      pool.query(`
        SELECT
          (SELECT COUNT(*) FROM forum_questions WHERE created_at > NOW()-INTERVAL '24 hours' AND is_visible=true) as posts_today,
...
// ─── POST /api/forum/questions/:id/pin — toggle pin (Chunk 4a) ───────────────
// Lawyer-only feature: pin one of your OWN forum posts to the top of your
// public profile (LinkedIn "Featured" pattern). Hard cap of 2 pinned posts
// per lawyer. Toggles: if currently pinned, unpins. If unpinned and under
// the cap, pins. If at cap and trying to pin a new one, returns 409 with a
// helpful message so the client can prompt the user to unpin one first.
router.post('/questions/:id/pin', requireAuth, async (req, res, next) => {
  try {
    // Defensive: ensure the column exists (migration 005)
    await pool.query(
      `ALTER TABLE forum_questions ADD COLUMN IF NOT EXISTS is_pinned BOOLEAN DEFAULT false`
    ).catch(() => {});

    const { rows: [post] } = await pool.query(
      `SELECT id, user_id, is_pinned FROM forum_questions WHERE id=$1 AND is_visible=true`,
      [req.params.id]
    );
    ...
    if (post.is_pinned) {
      // Toggle off
      await pool.query(`UPDATE forum_questions SET is_pinned=false WHERE id=$1`, [req.params.id]);
      return res.json({ pinned: false });
    }

    // Trying to pin — check cap
    const { rows: [{ pinned_count }] } = await pool.query(
      `SELECT COUNT(*) AS pinned_count FROM forum_questions
        WHERE user_id=$1 AND is_pinned=true AND is_visible=true`,
      [req.user.id]
    );
    if (parseInt(pinned_count) >= 2) {
      return res.status(409).json({
        message: 'You can only pin up to 2 posts. Unpin one first.',
        message_ar: 'يمكنك تثبيت منشورين فقط. الغِ تثبيت أحدهما أولاً.',
      });
    }

    await pool.query(`UPDATE forum_questions SET is_pinned=true WHERE id=$1`, [req.params.id]);
    res.json({ pinned: true });
  } catch (err) { next(err); }
});

// ─── GET /api/forum/users/:userId/pinned — get a user's pinned posts ─────────
// Public — anyone can fetch a user's pinned posts. Returns up to 2 rows.
router.get('/users/:userId/pinned', async (req, res, next) => {
  try {
    await pool.query(
      `ALTER TABLE forum_questions ADD COLUMN IF NOT EXISTS is_pinned BOOLEAN DEFAULT false`
    ).catch(() => {});
    ...
    const { rows } = await pool.query(
      `SELECT fq.*, u.name as asked_by, u.avatar_url as user_avatar_url, u.role as user_role,
       ...
       LEFT JOIN users u ON u.id=fq.user_id
       WHERE fq.user_id = $1
         AND fq.is_pinned = true
         AND fq.is_visible = true
       ORDER BY fq.created_at DESC
       LIMIT 2`,
      [req.params.userId]
    );
