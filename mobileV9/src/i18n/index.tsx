...
// ─── Snapshot RTL at boot ─────────────────────────────────────────────────────
// I18nManager.forceRTL() changes I18nManager.isRTL immediately in JS, but the
// NATIVE layout engine only applies it after a full restart. Reading isRTL from
// the live I18nManager after a language toggle gives the wrong (new) value while
// native still renders the old direction.
//
// Solution: read it ONCE at module load — before any forceRTL call — and freeze
// it. This matches the actual native rendering for the entire session.
const BOOT_IS_RTL = I18nManager.isRTL;
...
export function I18nProvider({ children }: { children: ReactNode }) {
  ...
  // isRTL: used ONLY for text alignment (textAlign/writingDirection), NOT for flexDirection.
  // Structural layouts (author row, action bar, sort tabs) always use 'row' — same as
  // Reddit, LinkedIn, Facebook — so they never flip regardless of language.
