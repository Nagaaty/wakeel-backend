...
export const firmsAPI = {
  ...
  create:     (data: { name: string; city?: string; website?: string; phone?: string; bar_registration_number?: string; document_url?: string }) => api.post('/firms', data),
...
export const usersAPI = { 
  ...
  // Autocomplete people search for forum top bar (Chunk 3b)
  search: (q: string) => api.get('/users/search', { params: { q } }),
  // Profile + activity (Chunk 4a)
  getProfileSummary: (id: string | number) => api.get(`/users/${id}/profile-summary`),
  getActivity: (id: string | number, params?: { before?: string; limit?: number }) =>
...
export const forumAPI = {
  // ⚡ Merged feed endpoint (Chunk 3a+) — questions + stats + trending in one call
  getFeed: (params?: { sort?: string; cat?: string; search?: string; limit?: number }) =>
    api.get('/forum/feed', { params }),
  getQuestions: (params?: { sort?: string; cat?: string; search?: string; tag?: string; before?: string; limit?: number }) =>
  ...
  sharePost: (id: string | number, body?: { repost_id?: number | string }) =>
    api.post(`/forum/questions/${id}/share`, body || {}),
  ...
  // Pin/unpin own posts (Chunk 4a — LinkedIn-style "Featured")
  pinPost: (id: string | number) =>
    api.post(`/forum/questions/${id}/pin`),
  getPinnedPosts: (userId: string | number) =>
    api.get(`/forum/users/${userId}/pinned`),
  // Comment likes
...
export const lawsAPI = {
  search: (params?: { search?: string; limit?: number; page?: number }) => api.get('/laws', { params }),
};
