...
export default function ResetPasswordScreen() {
  ...
  // ── Stage 2: just validate locally and move to password stage ───────────────
  const verifyOtp = (prebuiltCode?: string) => {
    const code = prebuiltCode ?? otp.join('');
    if (code.length < 6) {
      setError(isRTL ? 'أدخل الكود كاملاً (6 أرقام)' : 'Enter the full 6-digit code');
      return;
    }
    setError('');
    // Don't consume the OTP here — the backend /reset-password will verify+consume it atomically
  ...
  const handleOtpChange = (i: number, val: string) => {
    ...
    if (val && i === 5) {
      const full = [...next];
      if (full.every(d => d)) setTimeout(() => verifyOtp(full.join('')), 150);
    }
  };
  ...
  const resetPassword = async () => {
    ...
    setLoading(true); setError(''); setResetExpired(false);
    try {
      ...
      setError(isRTL ? 'انتهت صلاحية الكود. اطلب كوداً جديداً.' : 'Code expired. Please request a new code.');
    }
    finally { setLoading(false); }
  };
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 24 }}>
        ...
        <View style={{ backgroundColor: C.card, borderWidth: 1, borderColor: C.border, borderRadius: 20, padding: 24 }}>
          ...
          /* ── STAGE 2: OTP ────────────────────────────────────────── */
            ...
            <>
              {/* 6-box OTP Input */}
              <View style={{ flexDirection: 'row', direction: 'ltr', gap: 8, justifyContent: 'center', marginBottom: 24 }}>
                {otp.map((v, i) => (
                  <TextInput
                    key={i}
          ...
          /* ── STAGE 3: New Password ───────────────────────────────── */
            ...
            <>
              ...
              <Inp C={C}
                ...
                secureTextEntry />
              {/* Password match indicator — only show ❌ when confirm is as long as password */}
              {confirm.length > 0 && confirm.length >= password.length && (
              ...
              {/* Show 'get new code' link if the OTP was expired/consumed */}
              {resetExpired && (
                <TouchableOpacity
                  onPress={() => { setStage('email'); setError(''); setResetExpired(false); setOtp(['','','','','','']); setPassword(''); setConfirm(''); }}
                  style={{ marginTop: 14, alignItems: 'center' }}>
                  <Text style={{ color: C.gold, fontSize: 13, fontWeight: '700' }}>
                    {isRTL ? '⇦ اطلب كوداً جديداً' : '⇦ Request a new code'}
                  </Text>
                </TouchableOpacity>
              )}
            </>
