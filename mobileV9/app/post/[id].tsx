...
export default function PostDetail() {
  ...
  // ── Comment scroll-target tracking (Chunk 3c) ─────────────────────────
  // When a notification deep-links with ?commentId=N we want to scroll
  // to that exact comment after answers load. Each answer wraps its own
  // <View onLayout> that records its Y position into this ref Map.
  const answerYRef       = useRef<Map<number, number>>(new Map());
  const scrolledToCommentRef = useRef<boolean>(false);
  ...
  // ── Effect: scroll to deep-linked comment after answers load ───────────
  // Runs whenever the answers list updates. Tries to find the requested
  // comment's Y position and scroll there. Falls back to scrolling to the
  // bottom of the comments section if the position hasn't been measured yet
  // (onLayout hasn't fired). Re-tries up to 3 times with a short delay so
  // late layouts get caught.
  useEffect(() => {
    if (!commentId || scrolledToCommentRef.current || answers.length === 0) return;
    const target = parseInt(commentId, 10);
    if (!Number.isFinite(target)) return;

    let attempts = 0;
    const tryScroll = () => {
      attempts += 1;
      const y = answerYRef.current.get(target);
      if (typeof y === 'number') {
        // Subtract a small offset so the comment isn't flush against the
        // top of the screen — gives the user some context.
        scrollRef.current?.scrollTo({ y: Math.max(0, y - 60), animated: true });
        setHighlightedComment(target);
        scrolledToCommentRef.current = true;
        // Clear highlight after a couple seconds
        setTimeout(() => setHighlightedComment(null), 2400);
      } else if (attempts < 4) {
        // onLayout hasn't fired yet — wait and retry
              ...
              <View style={{ paddingHorizontal: 12, paddingBottom: 12, gap: 10 }}>
                {answers.map((a, idx) => (
                  <View
                    key={a.id || idx}
                    onLayout={(e) => {
                      // Cache Y so the deep-link scroll-to effect can find it
                      if (a.id) {
                        answerYRef.current.set(a.id, e.nativeEvent.layout.y);
                      }
                    }}
                    style={{
                      flexDirection: 'row', gap: 10, alignItems: 'flex-start',
                      // Brief gold halo if this is the comment a notification deep-linked to
                      backgroundColor: highlightedComment === a.id ? C.gold + '22' : 'transparent',
                      borderRadius: 12,
                      padding: highlightedComment === a.id ? 6 : 0,
                      margin: highlightedComment === a.id ? -6 : 0,
                    }}
                  >
