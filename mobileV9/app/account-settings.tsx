...
function SettingToggle({
  icon, title, subtitle, value, onValueChange, C, isRTL, disabled,
}: {
  icon: string; title: string; subtitle?: string;
  value: boolean; onValueChange: (v: boolean) => void;
  C: any; isRTL: boolean; disabled?: boolean;
}) {
  return (
    <View style={{
      flexDirection: isRTL ? 'row-reverse' : 'row',
      ...
      <View style={{ flex: 1 }}>
        <Text style={{ color: C.text, fontSize: 14, fontWeight: '700', textAlign: isRTL ? 'right' : 'left' }}>{title}</Text>
        {subtitle && (
          <Text style={{ color: C.muted, fontSize: 12, marginTop: 2, textAlign: isRTL ? 'right' : 'left' }} numberOfLines={2}>
            {subtitle}
          </Text>
        )}
      </View>
...
export default function AccountSettingsScreen() {
  ...
  // Privacy toggle (Chunk 4a)
  const [activityPublic, setActivityPublic] = useState<boolean>(
    user?.forum_activity_public ?? (user?.role === 'lawyer')
  );
  const [savingPrivacy, setSavingPrivacy] = useState(false);

  const togglePrivacy = async (v: boolean) => {
    setSavingPrivacy(true);
    setActivityPublic(v); // optimistic
    try {
      await authAPI.update({ forum_activity_public: v });
    } catch (e) {
      // Revert on failure
      setActivityPublic(!v);
      Alert.alert(
        isRTL ? 'خطأ' : 'Error',
        isRTL ? 'تعذّر حفظ الإعداد. حاول مجدداً.' : 'Could not save the setting. Please try again.',
      );
    } finally {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 60 }}>
        ...
        {/* ── Privacy (Chunk 4a) ─────────────────────────────────────── */}
        <SectionHeader label={isRTL ? 'الخصوصية' : 'Privacy'} C={C} isRTL={isRTL} />
        <View style={{ backgroundColor: C.surface, borderTopWidth: 1, borderBottomWidth: 1, borderColor: C.border }}>
          <SettingToggle
            icon="🌐" C={C} isRTL={isRTL}
            title={isRTL ? 'الملف الشخصي العام' : 'Public Profile'}
            subtitle={isRTL
              ? (activityPublic
                  ? 'مفعّل — يمكن للآخرين رؤية نشاطك ومعلوماتك العامة.'
                  : 'معطّل — ملفك خاص حالياً. لا يرى أحد نشاطك أو منشوراتك.')
              : (activityPublic
                  ? 'Enabled — Others can see your activity and public info.'
                  : 'Disabled — Your profile is private. Nobody sees your posts or activity.')}
          ...
          {/* Hint for clients */}
          {user?.role !== 'lawyer' && (
            <View style={{ paddingHorizontal: 20, paddingVertical: 10, flexDirection: 'row', alignItems: 'flex-start', gap: 8 }}>
              <Text style={{ fontSize: 14 }}>{activityPublic ? '🔓' : '🔒'}</Text>
              <Text style={{ color: C.muted, fontSize: 12, flex: 1, lineHeight: 18 }}>
                {isRTL
                  ? (activityPublic
                      ? 'ملفك مرئي للجميع. يمكن للمحاميين والمستخدمين رؤية اسمك ونشاطك.'
                      : 'ملفك خاص. سيرى الآخرون فقط اسمك وصورتك — بدون أي تفاصيل أخرى.')
                  : (activityPublic
                      ? 'Your profile is visible to everyone. Lawyers and users can see your name and activity.'
                      : 'Your profile is private. Others will only see your name and photo — nothing else.')}
              </Text>
            </View>
          )}
        </View>
